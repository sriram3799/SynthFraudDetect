{{
  config(
    materialized = 'table',
    description  = 'Cleaned and typed enriched transaction events from the Iceberg lake. '
                   'Drops any rows with a NULL transaction_id and normalises timestamps to TIMESTAMPTZ.'
  )
}}
...
with source as (
    ...
    select *
    from {{ source('sentinel_raw', 'transactions') }}
